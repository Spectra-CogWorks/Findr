...
def cosine_similarity(d1, d2):
    ...
    )  # / (norm(d1) * norm(d2)) - For non-unit vectors
...
class COCO:
    ...
    def init_annotation_mappings(self):
        ...
        for caption_dict in self.database["annotations"]:
            ...
            if caption_dict["image_id"] in self.img_ids_to_caption_ids:
                self.img_ids_to_caption_ids[caption_dict["image_id"]].append(
                    caption_dict["id"]
                )
            else:
                self.img_ids_to_caption_ids[caption_dict["image_id"]] = [
                    caption_dict["id"]
                ]

            if caption_dict["image_id"] in self.img_ids_to_captions:
                self.img_ids_to_captions[caption_dict["image_id"]].append(
                    caption_dict["caption"]
                )
            else:
                self.img_ids_to_captions[caption_dict["image_id"]] = [
                    caption_dict["caption"]
                ]
    ...
    def inverse_document_frequency(self, documents: Iterable[str]) -> Dict[str, int]:
        ...
        return {
            word: np.log(len(documents) / (1.0 + count)) for word, count in df.items()
        }
    ...
    def find_similar_images(self, model, query, k):
        ...
        for image in self.database["images"]:
            ...
            if img_descriptor is not None:
                ids_to_scores[image["id"]] = cosine_similarity(
                    query, model(img_descriptor)
                )
        ...
        return [k for k, v in sorted(ids_to_scores.items(), key=lambda item: item[1])][
            -k:
        ]
