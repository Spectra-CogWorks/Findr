    ...
    for good_cap in caption_ids:
        ...
        while len(bad_caps) < 10:
            ...
            for bad_cap in bad_batch_caps:
                if bad_cap in cos_sims.values() or coco.get_image_id(
                    bad_cap
                ) == coco.get_image_id(good_cap):
                    continue
                else:
                    cos_sim = cosine_similarity(
                        coco.get_caption_embedding(good_cap),
                        coco.get_caption_embedding(bad_cap),
                    )
        ...
        truple = [
            (
                generate_descriptor(coco.get_image_id(good_cap)),
                coco.get_caption_embedding(good_cap),
                generate_descriptor(coco.get_image_id(bad_cap)),
            )
